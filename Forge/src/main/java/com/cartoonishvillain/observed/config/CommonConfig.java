...
public class CommonConfig {
    ...
    public static final String CATEGORY = "ObserverModel Properties";
    ...
    public CommonConfig(ForgeConfigSpec.Builder builder, ConfigHelper.Subscriber subscriber){
        ...
        this.CLOSEOBSERVERATE = subscriber.subscribe(builder.comment("Rate of observation point gain per second when the target is within initial 30% of the observer's range. (for 32, this would be within 9.6 blocks)").defineInRange("closeObserveRate", 0.5, 0.25, 2));
        this.NEAROBSERVERATE = subscriber.subscribe(builder.comment("Rate of observation point gain per second when the target is out of 30% range but in 60% range. (for 32, this would be further than 9.6 blocks, but within 19.2 blocks)").defineInRange("nearObserveRate",0.35, 0.1, 1));
        this.FAROBSERVERATE = subscriber.subscribe(builder.comment("Rate of observation point gain per second when the target is out of 60% range").defineInRange("farObserveRate", 0.2, 0.05, 0.75));
